...
if __name__ == "__main__":
    with open("../outputs/csv_vasp/results.csv", mode="w") as file:
    ...
    calc = PyHighThroughput(
        input_path="../../../pythroughput/default_pot/vasp/",
        output_path="../outputs/calc_vasp/",
        **structs)
    calc.run(package="vasp")
    atomization = AtomizationCalculator(calc.results, ["O", "F", "Fe"])
    atomization.get_atomization_energy(package="vasp", input_path="../../../pythroughput/default_pot/vasp/")
    
    with open("../outputs/csv_vasp/results.csv", mode="a") as file:
        writer = csv.writer(file, lineterminator="\n")
